...
from API-Football and upsert them into the SQLite database.

You run this rarely: once to grab history, then once a week to add new results.
The prediction model itself never touches the API — it works off the saved DB.
...
def pull_fixtures():
    ...
    print(f"\nPulled {len(rows)} fixtures ({completed} completed). "
          f"Added {added} new, {skipped} already existed.")
